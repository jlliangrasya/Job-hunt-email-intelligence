...
import { OPPORTUNITY_TYPES, getDomainConfig, getStatusMeta } from "./domain-config";
...
/**
 * Highest score the formula can produce: the largest status weight across all
 * types plus both bonuses at their cap. UI meters normalize against this so the
 * bar stays meaningful when new types or weights are added to domain-config.
 */
export function maxPriorityScore() {
  const weights = Object.values(OPPORTUNITY_TYPES).flatMap((config) =>
    Object.values(config.priorityWeights ?? {})
  );
  return Math.max(0, ...weights) + RECENCY_MAX_BONUS + OVERDUE_MAX_BONUS;
}
