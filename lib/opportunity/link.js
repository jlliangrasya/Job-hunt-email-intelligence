...
/**
 * Surface a PostgREST error instead of discarding it.
 *
 * Destructuring only `data` — the shape this file used to be written in — turns
 * every rejected write into a silent no-op: a scan reports "0 detected" and a
 * webhook does nothing, both indistinguishable from a mailbox with no
 * applications in it. That is precisely how an unapplied migration went
 * unnoticed; a missing column has to fail loudly.
 */
...
export async function findOpportunityByThread(supabase, userId, threadId, columns = OPPORTUNITY_COLUMNS) {
  ...
  return unwrap(result, "Thread lookup") ?? null;
}
