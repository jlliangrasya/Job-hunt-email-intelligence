...
/**
 * Log a rejected write rather than discarding it. The webhook has no user
 * watching it, so a swallowed PostgREST error is invisible forever — the same
 * failure mode that let a missing column look like an inbox with no job mail in
 * it. Processing continues: the daily rescan re-covers whatever did not land.
 */
async function logged(query, action) {
  const { data, error } = await query;
  if (error) console.error(`${action} failed:`, error.message);
  return data;
}
...
async function detectFromInbound(supabase, userId, msg, type = "job") {
  ...
  await logged(
    supabase.from("interaction_events").upsert(
      {
        user_id: userId,
...
export async function processWebhookEvent(emailAddress, incomingHistoryId) {
  ...
  for (const addition of additions) {
    ...
    // No single message may abort the event: the loop would exit before the
    // history cursor advances, so one message the classifier or the database
    // chokes on would stall processing for every later message indefinitely.
    // The daily rescan (app/api/cron/scan-sent-mail) re-covers detection, so a
    // message dropped here is recovered rather than lost.
    try {
      await processMessage(supabase, userId, messageId);
    } catch (e) {
      console.error(`Webhook message ${messageId} failed:`, e);
    }
  }
...
/** Classify one new message as a reply to a tracked opportunity, or detect a new one. */
async function processMessage(supabase, userId, messageId) {
  ...
  await logged(
    supabase.from("opportunities").update(update).eq("id", opportunity.id),
