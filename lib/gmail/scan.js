  ...
  for (const pass of passes) {
    ...
    for (let i = 0; i < pass.messageIds.length; i += BATCH_SIZE) {
      ...
      const emailInputs = messages.map((res) => ({
        ...
        snippet: normalizeSnippet(res.data.snippet),
      }));
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        // One unwritable message must not abort a scan of several hundred, but
        // it must not vanish either: a rejected write is counted and reported,
        // so "found nothing" stays distinguishable from "could not save".
        try {
          ...
          failed++;
          firstError ??= e;
          console.error("Opportunity write failed:", e);
        }
      }

      scanned += batch.length;
      yield { scanned, total, detected, failed };
    }
  }

  // Every detection failing is a broken deployment — an unapplied migration, a
  // revoked grant — not an empty mailbox. Marking discovery complete there would
  // hide the breakage behind a finished-looking onboarding, so the scan reports
  // the failure and leaves the flag alone for the next run to retry.
  if (failed > 0 && detected === 0) {
    throw new Error(
      `Scan could not save any of ${failed} detected opportunities: ${firstError?.message}`
    );
  }
