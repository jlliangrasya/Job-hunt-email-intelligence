  ...
  for (let i = 0; i < messageIds.length; i += 5) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      if (cls?.isOpportunity && cls.confidence > 0.75) {
        ...
        const { data: opportunity } = await supabase.from("opportunities").upsert(
          {
            user_id: userId,
            type,
            channel: "email",
            ...
            initiated_at:
              toDateOnly(cls.initiatedAt) ??
              toDateOnly(emailInputs[j].date) ??
        ...
        if (opportunity) {
          const parsedDate = new Date(emailInputs[j].date);
          await supabase.from("interaction_events").upsert(
            {
              ...
              channel_thread_id: msg.threadId,
              direction: "sent",
              to_addresses: [emailInputs[j].to],
              subject: emailInputs[j].subject,
              snippet: emailInputs[j].snippet.slice(0, 500),
              received_at: Number.isNaN(parsedDate.getTime())
